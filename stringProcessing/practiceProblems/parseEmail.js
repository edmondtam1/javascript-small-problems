...
var DAY_CONVERSION = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
var MONTH_CONVERSION = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

function mailCount(emailData) {
  var emails = [];

  emailData.split('##||##').forEach(email => {
    var emailSections = email.split('#/#');
    emails.push({
      sender: emailSections[0],
      subject: emailSections[1],
      date: emailSections[2],
      recipient: emailSections[3],
      body: emailSections[4],
    })
  })

  var emailCount = emails.length;
  var dateRange = calculateDateRange(emails);

  console.log(`Count of Email: ${emailCount}`);
  console.log(`Date Range: ${dateRange[0]} - ${dateRange[1]}`);
}

function calculateDateRange(emails) {
  var minDate = new Date();
  var maxDate = new Date(-8640000000000000);

  emails.forEach(email => {
    var date = new Date(email.date.match(/\d{2}-\d{2}-\d{4}/));
