...
function sentiment(text) {
  ...
  text.toLowerCase().split(/[^a-z0-9']/gi).forEach(word => {
  ...
  logText(positives, 'positive');
  logText(negatives, 'negative');
...
function logText(array, sentiment) {
  console.log(`There are ${array.length} ${sentiment} words in the text.`);
  console.log(`${sentiment[0].toUpperCase() + sentiment.slice(1)} sentiments: ${array.join(', ')}`)
}
